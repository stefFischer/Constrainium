...
public class RestSystemDriverPayrollTest {
    ...
    public void postEmployeeRequestBodyTest() throws DriverException {
        ...
        InOutputDataCollection expected = InOutputDataCollection.parseData(
                new Pair<>("{employee:{name:\"Alice\", role:\"Engineer\"}}", "{id:3,name:\"Alice\", role:\"Engineer\"}"),
                new Pair<>("{employee:{name:\"Bob\", role:\"Manager\"}}", "{id:4,name:\"Bob\", role:\"Manager\"}")
        );
    ...
    public void putEmployeePathAndBodyTest() throws DriverException {
        ...
        SimpleDataCollection input = SimpleDataCollection.parseData(
                "{id:1, employee:{name:\"Alice\", role:\"Architect\"}}",
                "{id:2, employee:{name:\"Bob\", role:\"Director\"}}"
        );

        InOutputDataCollection expected = InOutputDataCollection.parseData(
                new Pair<>("{id:1, employee:{name:\"Alice\", role:\"Architect\"}}", "{id:1, name:\"Alice\", role:\"Architect\"}"),
                new Pair<>("{id:2, employee:{name:\"Bob\", role:\"Director\"}}", "{id:2, name:\"Bob\", role:\"Director\"}")
        );
