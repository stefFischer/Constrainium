...
//		));
		...
		// TODO The same values appear from multiple constraints in OneOfString, which looks like when it is replacing the variables it still references to the same array of values.
...
//		));
		...
		// TODO This shows some inconsistencies: (size(input.hotelIds[..]), size(return.null.data[..])) & (size(input.hotelIds[..])-1, size(return.null.data[..])-1) should be the same, yet we do not get the same result for them.
		//  Why is that? It seems there is some non-determinism going on.
...
//		));
	...
	// TODO AmadeusHotel hotelIds is encoded as a URL. e.g., "BLPAR123%2CMCPHLDTM%2CARMADAIT%2CHYHAMIPH" where "%2" is "," for the array of ids. Need to think of a way to parse this.
...
//		TODO Do we need to parse the data again and cast the types to the ones in the schema? Or does it ignore the wrong type already?
		...
		invariantData.visitRows(new ValueTableRowVisitor() {
			...
			public void visit(ValueTableRow row) {
				...
				if(!isCovered){
					...
					System.out.println("check values: " + List.of(variables));
